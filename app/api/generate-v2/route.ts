...
3. 주당 sessions 배열 개수 = 사용자의 주당 훈련 가능 일수.
   단, 마지막 주는 user 메시지의 "[마지막 주 구조]"에 명시된 세션 수를 반드시 따른다.
...
- LSD 상한선: user 메시지의 "[LSD 거리 기준]" lsdPeakKm 값을 따른다. 임의 수정 금지.
- LSD 매주 2km 이상 증가 금지
...
- 단기 플랜 LSD 상한: user 메시지의 "[LSD 거리 기준]" lsdPeakKm 값을 따른다. 임의 수정 금지.
- 볼륨 급증 금지: 단기 플랜에서도 주간 거리 전주 대비 10% 초과 증가 금지를 동일하게 적용한다. 짧은 기간이라고 예외 없음.
- 이지런 최솟값 준수: user 메시지의 "[이지런·볼륨 제약]"에 명시된 최솟값 이하의 이지런 계획 금지.
  볼륨이 초과할 경우 이지런 수(세션 개수)를 줄여 조정하되, 세션당 거리는 최솟값 이하로 축소 절대 금지.
- 마지막 주 세션 수: user 메시지의 "[마지막 주 구조]"에 명시된 훈련 가능 날짜 수 = sessions 배열 개수.
  해당 주 대회 전날은 코드가 rest 처리하므로 sessions에 포함하지 말 것.
...
function normalizePlan(
  ...
  for (let weekIdx = 0; weekIdx < plan.weekly_plans.length; weekIdx++) {
    ...
    for (const day of week.days) {
      ...
      // 이지런 최솟값 강제: LLM이 너무 짧게 계획해도 보정
      if (day.session_type === "easy" && easyMinKm !== undefined && (day.distance_km ?? 0) < easyMinKm) {
        day.distance_km = easyMinKm;
      }
    ...
    // 2) 볼륨 캡: easy 세션만 비례 축소 (단, 세션당 최솟값 보장)
    if (weekVolumeCap !== undefined) {
      ...
      if (weekTotal > weekVolumeCap && easyKm > 0) {
        ...
        // 세션당 최솟값 보장 — 최솟값 총합보다 budget이 낮으면 그대로 유지
        const minEasyTotal = easyDays.length * (easyMinKm ?? 0);
        const effectiveBudget = Math.max(easyBudget, minEasyTotal);
        const scale = effectiveBudget / easyKm;
        for (const day of easyDays) {
          const scaled = Math.round((day.distance_km ?? 0) * scale * 10) / 10;
          day.distance_km = easyMinKm !== undefined ? Math.max(scaled, easyMinKm) : scaled;
        }
        console.log(`[volume-cap] week ${week.week}: ${weekTotal.toFixed(1)}km → capped (easy ×${scale.toFixed(2)}, minPerSession=${easyMinKm ?? 0}km)`);
      }
    }
...
function assignDatesToPlan(
  ...
  const weeks: GeneratedWeek[] = v2Plan.weekly_plans.map((v2Week, weekIdx) => {
    ...
    // 3. 미배정 훈련 가능 날짜 자동 채우기 (이지런)
    // — 마지막 주가 7일 이상일 때 LLM 계획 세션보다 날짜가 많은 경우 보정
    if (autoEasyPaceSec !== undefined && easyMinKm !== undefined) {
      const unassigned = trainingDates.filter((d) => d !== raceDate && !assignment.has(d));
      if (unassigned.length > 0) {
        console.log(`[assignDates] 미배정 훈련일 ${unassigned.length}개 → 이지런 자동 추가 (week ${v2Week.week})`);
        for (const date of unassigned) {
          assignment.set(date, {
            session_type: "easy",
            title: "이지런",
            distance_km: easyMinKm,
            pace_target: toMinSec(autoEasyPaceSec),
            hr_zone: "Z2",
            duration_min: Math.round((easyMinKm * autoEasyPaceSec) / 60),
            sets: null,
            warmup: null,
            cooldown: null,
            tempo_segment: null,
    ...
    if (isLastWeek) {
      ...
      console.log("[assignDates] assignment keys:", [...assignment.keys()]);
    }

    // 4. raceDate는 항상 race 세션으로 강제 배정 (LLM 출력 무관, 허용 요일 무관)
...
export async function POST(request: Request) {
  ...
  // 실제 남은 일수 직접 계산 (validate 시점과 시차 방지)
  const daysUntilRace = Math.max(0, Math.floor(
    (new Date(body.formData.raceDate + "T00:00:00").getTime() - new Date(today + "T00:00:00").getTime()) / (1000 * 60 * 60 * 24)
  ));
  console.log("[/api/generate-v2] today:", today, "| daysUntilRace:", daysUntilRace);
...
`.trim();
  ...
  // 주차 수 계산
  // validate의 total_weeks(floor 기반)에서 남은 일수가 4일 이상이면 추가 주차 생성
  // — 이렇게 하면 마지막 주가 14일 이상으로 늘어나는 버그를 방지한다
  const totalWeeks = body.validation.validation.training_period.total_weeks;
  const remainderDays = daysUntilRace % 7;
  const effectiveTotalWeeks = remainderDays >= 4 ? totalWeeks + 1 : totalWeeks;
  const isShortPlan = effectiveTotalWeeks <= 4;

  // 이지런 최소 거리: 현재 주간 볼륨 대비 훈련일당 평균 × 0.7 (최소 4km)
  const easyMinKm = Math.max(
    4,
    Math.round(body.formData.weeklyMileage1 / Math.max(body.formData.trainingDays.length, 1) * 0.7 * 10) / 10
  );

  // 주간 볼륨 상한 (단기 플랜 전용 — LLM과 normalizePlan 양쪽에 동일 기준 사용)
  const weekVolumeCap = isShortPlan
    ? Math.round(Math.max(body.formData.weeklyMileage1, body.formData.weeklyMileage4 / 4) * 1.1 * 10) / 10
    : undefined;

  // 마지막 주 실제 훈련 가능 세션 수 계산 (LLM에 정확한 세션 수 안내)
  const DAY_NUM_MAP: Record<string, number> = { sun: 0, mon: 1, tue: 2, wed: 3, thu: 4, fri: 5, sat: 6 };
  const allowedDowsSet = new Set(body.formData.trainingDays.map((d) => DAY_NUM_MAP[d]));
  const lastWeekStart = new Date(today + "T00:00:00");
  lastWeekStart.setDate(lastWeekStart.getDate() + (effectiveTotalWeeks - 1) * 7);
  const preRaceDateStr = localDateStr(new Date(new Date(body.formData.raceDate + "T00:00:00").getTime() - 86400000));
  let lastWeekEligibleCount = 0;
  {
    const cursor = new Date(lastWeekStart);
    while (localDateStr(cursor) < body.formData.raceDate) {
      const ds = localDateStr(cursor);
      if (ds !== preRaceDateStr && allowedDowsSet.has(cursor.getDay())) {
        lastWeekEligibleCount++;
      }
      cursor.setDate(cursor.getDate() + 1);
    }
  }
  console.log("[/api/generate-v2] effectiveTotalWeeks:", effectiveTotalWeeks, "| lastWeekEligibleCount:", lastWeekEligibleCount, "| easyMinKm:", easyMinKm, "| weekVolumeCap:", weekVolumeCap);

  // LSD 상한: 단기 플랜도 유저의 실제 maxRunDistance 반영 (기존 하드코딩 10km 제거)
  ...
  // 단기 플랜 LSD 상한: maxRunDistance × 0.8 이하, 15km 이하
  const shortPlanLsdCap = Math.min(
    Math.round(maxRun * 0.8 * 10) / 10,
    15
  );
  const raceTypeLsdCeil: Record<string, number> = {
    "5k": isShortPlan ? shortPlanLsdCap : 15,
    "10k": isShortPlan ? shortPlanLsdCap : 15,
    half: 24,
    full: 35,
  };
  ...
  lsdPeakKm = Math.max(lsdPeakKm, peakFloorKm);
  lsdPeakKm = Math.max(lsdPeakKm, lsdStartKm);

  // 주차별 LSD 스케줄 계산 (3+1 빌드/회복 사이클, effectiveTotalWeeks 기준)
  const buildWeeks = Math.max(1, effectiveTotalWeeks - 2);
  const lsdSchedule: number[] = [];
  let buildLsd = lsdStartKm;
  ...
  // Easy 페이스 3구간 계산 (effectiveTotalWeeks 기준)
  ...
  // 테이퍼 시작 주차 (effectiveTotalWeeks 기준)
  const taperStartWeekNum = isShortPlan ? effectiveTotalWeeks + 1 : effectiveTotalWeeks - 1;

  const taperSection = isShortPlan
    ? `[테이퍼링 — 단기 플랜 예외 적용]
taper_start_week: ${taperStartWeekNum} (테이퍼 없음. 전 주차 인터벌 1회/주 허용)
대회 전날(${effectiveTotalWeeks}주차 마지막): 하루만 rest. 그 외 테이퍼 규칙 일절 적용 안 함.`
...
대회 1주 전 (${effectiveTotalWeeks}주차): 볼륨 60% 감소, 인터벌 없음, easy만`;
...
[이지런·볼륨 제약 — 반드시 준수]
이지런 세션 최소 거리: ${easyMinKm}km (이 값 이하 이지런 절대 금지)
주간 볼륨 상한: ${weekVolumeCap ?? "없음"}km (초과 시 이지런 수를 줄이되 세션 거리 ${easyMinKm}km 이하 축소 금지)

[플랜 주차 수 — 반드시 이 값을 사용할 것]
총 ${effectiveTotalWeeks}주 계획 (weekly_plans 배열 길이 = ${effectiveTotalWeeks})

[마지막 주 구조 — 반드시 준수]
마지막 주(${effectiveTotalWeeks}주차) 훈련 가능 세션 수: ${lastWeekEligibleCount}개
sessions 배열 개수 = ${lastWeekEligibleCount} (대회 당일 race 세션 포함, 대회 전날 rest는 제외하고 계획)
`;
  ...
  const stream = new ReadableStream({
    async start(controller) {
      try {
        ...
        // taper_start_week 강제 오버라이드
        v2Plan.plan_summary.taper_start_week = taperStartWeekNum;

        const finalPlan = assignDatesToPlan(v2Plan, body.formData, today, easySec, easyMinKm);
        ...
        // hr_zone 고정, 인터벌 거리 재계산, 이지런 최솟값 강제, 볼륨 캡, stats 재계산
        normalizePlan(finalPlan, weekVolumeCap, lsdSchedule, body.formData.raceType, easyMinKm);
