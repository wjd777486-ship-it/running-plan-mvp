...
function getLogger() {
  ...
  return {
    ...
    event: (groupId, params) => {
      ...
      loggerFunction?.({
        log_type: "event",
        log_name: log_name ?? createLogName(groupId, params.event_type),
        params: otherParams
      });
    }
  };
...
function LoggingEvent({
  ...
  return cloneElement(child, {
    [capture]: async (...args) => {
      ...
      const originalHandler = typeof childProps[capture] === "function" ? childProps[capture] : null;
...
function getReactInternalFiber(component) {
  const fiber = component?.__internalInstanceHandle || component?._reactInternals || component?._reactInternalFiber || component?._internalInstanceHandle;
  return fiber;
}
